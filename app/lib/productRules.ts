...
const HERO_BACKGROUND =
  "FIXED TEMPLATE for this shot only — always use exactly this, ignore the room styling and theme prop sections. This exact framing, crop, and background must be produced identically every single time this shot is generated, for every product type, with zero variation. Extreme close-up hero product shot, shot on a tight lens/zoomed in: the product must fill practically the entire frame edge to edge on EVERY side (roughly 98%+ of both the frame height AND the frame width — this applies to square, portrait, and landscape crops alike) with only a hairline sliver of background margin, barely a few pixels, on each side — crop in extremely aggressively, this is NOT a small product floating with lots of empty space around it, and it must look noticeably larger/tighter than a standard product photo. Centered in frame, set against a seamless pure white studio background (plain white, not grey, not cream, not off-white) — no room setting, no theme props, no decor of any kind. Soft, even, shadowless studio lighting. No hands. No clutter. No other objects in frame.";
