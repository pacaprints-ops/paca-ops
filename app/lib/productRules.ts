...
const HERO_BACKGROUND =
  "FIXED TEMPLATE for this shot only — always use exactly this, ignore the room styling and theme prop sections. This exact framing, crop, and background must be produced identically every single time this shot is generated, for every product type, with zero variation. Macro/extreme-close-up product shot: zoom in so tight that the product+envelope group barely fits in frame — it is CORRECT and EXPECTED for the very top edge of the envelope and/or the very bottom edge of the product to be slightly cropped off by the top/bottom of the frame. A small amount of cropping on the product itself is the desired result here, not a mistake — the mistake to avoid is leaving any visible band of empty white background space above, below, or beside the product. If you find yourself deciding between leaving a bit of background margin and cropping slightly into the product/envelope — always choose to crop into the product. This is deliberately much tighter than a normal product photo — treat any comfortable, breathing-room framing as wrong. Centered in frame, set against a seamless pure white studio background (plain white, not grey, not cream, not off-white) — no room setting, no theme props, no decor of any kind. Soft, even, shadowless studio lighting. No hands. No clutter. No other objects in frame.";
...
const HERO_ENVELOPE =
  "Exact staging, matching the approved reference photo — this exact composition must be produced identically every single time this shot is generated, for every product that has an envelope, with zero variation. A plain white envelope stands leaning back against the studio background, its bottom edge resting on the surface directly behind where the product will sit; its triangular back flap point is visible above the top edge of the product. ORIENTATION — MANDATORY: the envelope is always in strict portrait/vertical orientation, standing upright taller than it is wide, exactly like a portrait card envelope — NEVER landscape, NEVER rotated onto its side, NEVER lying flat, NEVER wider than it is tall. The product then leans back directly against the flat front face of that envelope, its own bottom edge also resting on the surface a little in front of the envelope's — the back of the product must be in physical contact with the front of the envelope, touching with zero gap between them. Never place the envelope elsewhere in the scene, never let it float or hover, never let the product stand fully upright/vertical away from the envelope, and never separate the two — they lean against each other as one connected pair on the same surface. CENTERING: treat the product+envelope pair as a single combined group and center that whole group horizontally and vertically in the frame, with equal empty space to its left and right — do not center the product alone and let the envelope's peek push the group off to one side.";
