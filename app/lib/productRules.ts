...
const HERO_BACKGROUND =
  "FIXED TEMPLATE for this shot only — always use exactly this, ignore the room styling and theme prop sections. This exact framing, crop, and background must be produced identically every single time this shot is generated, for every product type, with zero variation. FRAMING (measured from the approved reference photo — match it precisely): the product (plus its envelope, if it has one) fills about three-quarters (roughly 75-80%) of the frame's height and width, positioned centrally, with a modest, even margin of plain background around it on every side — roughly the same amount of visible background as the approved reference photo, no more and no less. Set against a seamless pure white studio background (plain white, not grey, not cream, not off-white) — no room setting, no theme props, no decor of any kind. Soft, even, shadowless studio lighting. No hands. No clutter. No other objects in frame.";
