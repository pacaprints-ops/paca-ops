...
const HERO_BACKGROUND =
  "FIXED TEMPLATE for this shot only — always use exactly this, ignore the room styling and theme prop sections. This exact framing, crop, and background must be produced identically every single time this shot is generated, for every product type, with zero variation. Extreme close-up hero product shot, shot on a tight lens/zoomed in: the product must fill practically the entire frame edge to edge (roughly 95%+ of the frame height, top and bottom edges of the product almost touching the top and bottom of the image) with only a hairline sliver of background margin on each side — crop in aggressively, this is NOT a small product floating with lots of empty space around it. Centered in frame, set against a seamless pure white studio background (plain white, not grey, not cream, not off-white) — no room setting, no theme props, no decor of any kind. Soft, even, shadowless studio lighting. No hands. No clutter. No other objects in frame.";
