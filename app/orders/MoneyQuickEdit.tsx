...
export default function MoneyQuickEdit({
  orderId,
  currentValue,
  title,
  label,
  ...
  label: string;
  rpcName: string;
  rpcParam: string; // e.g. "p_gross_revenue"
}) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [val, setVal] = useState<string>(String(currentValue ?? 0));
  ...
  async function save() {
    setErrorMsg("");
    if (parsed === null) return setErrorMsg("Value must be a number.");
